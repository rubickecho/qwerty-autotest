...
test.describe('Dictionary manage', () => {
  ...
  test('Switch language', async ({ page }) => {
    ...
    await expect(await page.getByRole('button', { name: /日语常见词/g }).first().isVisible()).toBeTruthy();

    await page.getByRole('radio', { name: /^Code$/ }).click();
    await expect(await page.getByRole('radio', { name: /^Code$/ }).getAttribute('aria-checked')).toBeTruthy();
    await expect(await page.getByRole('button', { name: /Coder Dict/g }).first().isVisible()).toBeTruthy();
  });
  ...
  test('Switch dictionary', async ({ page }) => {
    await page.getByRole('button', { name: /六级巧记速记/g }).first().click();
